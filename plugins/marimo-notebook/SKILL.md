...
This authoring workflow is shared across CC, Codex, and OMP. Use the host's native file tools. Resolve bundled references relative to this SKILL.md. If starting a marimo server is requested, use the host's supervised/background process interface, record the process identity, and check readiness; a shell launch alone does not establish a working notebook.
