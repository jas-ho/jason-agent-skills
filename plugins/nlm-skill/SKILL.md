---
name: nlm-skill
description: 'Expert guide for the NotebookLM CLI (`nlm`) and MCP server. Use for creating/managing notebooks, adding sources, generating content (podcasts, reports, quizzes, flashcards, mind maps, slides, videos), or chatting with sources. Triggers on "nlm", "notebooklm", "podcast generation", "audio overview", or any NotebookLM automation task.'
metadata:
  version: "0.5.29"
---
...
1. **Check for MCP tools**: Discover available NotebookLM tools by provider/operation; MCP prefixes differ across harnesses
2. **If BOTH MCP tools AND CLI are available**: Honor an existing preference; otherwise use the CLI for this shared workflow. Ask only when the difference materially affects the requested operation
...
if has_mcp_tools and has_cli:
    # Honor an existing preference; default to CLI when equivalent.
    user_preference = existing_preference_or_cli()
...
1. **Check existing authentication**: Use a harmless supported status/list operation first. Run the supported login only when authentication is missing/expired; new account choices or browser consent need the user. Never copy auth storage between harnesses.
